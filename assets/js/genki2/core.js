...
const soundIsOn = () => {
  try { return localStorage.getItem("eastokyo-audio-v4") !== "off"; }
  catch { return true; }
};
...
export function createGenki2({ unit, dialogue, moodLabel, patienceLabel, scoreLabel, soundButton }) {
  ...
  function queueIdleLine() {
    ...
    idleTimer = window.setTimeout(() => {
      if (!state.speaking && !document.hidden) speak(randomItem(idleLines), Math.random() > .72 ? "angry" : "curious");
  ...
  unit?.addEventListener("keydown", (event) => {
    if (event.key === "Enter" || event.key === " ") { event.preventDefault(); reactToTap(); }
  });
