(() => {
  ...
  const ASSET_PAYLOAD = new URL("../../images/genki2/genki2-canonical-fixed.webp.b64", import.meta.url).href;
  ...
  style.dataset.genki2Styles = "canonical-v2";
  style.textContent = `
    ...
    @media(max-width:799px){body.genki2-always-present main{padding-top:var(--genki2-mobile-stage)!important}body.genki2-always-present footer{position:relative;z-index:1}.genki2-global-presence{left:0!important;right:0!important;top:0!important;bottom:auto!important;width:100%!important;height:var(--genki2-mobile-stage)!important;transform:none!important;border:0;border-bottom:1px solid rgba(91,220,255,.22);border-radius:0;box-shadow:0 18px 40px rgba(0,0,0,.45)}body.genki2-has-system-bar .genki2-global-presence{top:64px!important}body.genki2-has-system-bar main{padding-top:calc(var(--genki2-mobile-stage) + 64px)!important}.genki2-global-presence__figure{width:164px;height:444px;bottom:-258px}.genki2-global-presence__image{object-position:50% 11%}.genki2-global-presence__mouth-light{top:10.5%;width:15px;height:5px}.genki2-arrival__figure{height:82vh;width:78vw;object-position:50% 26%}}
  ...
  presence.innerHTML = `<div class="genki2-global-presence__city" aria-hidden="true"></div><div class="genki2-global-presence__glow" aria-hidden="true"></div><button class="genki2-global-presence__figure" type="button" aria-label="Interact with Genki2"><img class="genki2-global-presence__image" alt="" draggable="false"><span class="genki2-global-presence__mouth-light" aria-hidden="true"></span></button>`;
  document.body.appendChild(presence);

  const arrival = document.createElement("div");
  arrival.className = "genki2-arrival";
  arrival.setAttribute("aria-hidden", "true");
  arrival.innerHTML = `<img class="genki2-arrival__figure" alt="">`;
  document.body.appendChild(arrival);

  fetch(ASSET_PAYLOAD, { cache: "no-store" })
    .then((response) => {
      if (!response.ok) throw new Error(`Genki2 asset payload failed: ${response.status}`);
      return response.text();
    })
    .then((payload) => {
      const source = `data:image/webp;base64,${payload.trim()}`;
      presence.querySelector(".genki2-global-presence__image").src = source;
      arrival.querySelector(".genki2-arrival__figure").src = source;
    })
    .catch((error) => {
      console.error(error);
      presence.remove();
      arrival.remove();
      document.body.classList.remove("genki2-always-present", "genki2-has-system-bar");
    });
